...
public class JSF31KochFractalFX extends Application {
    ...
    // Labels and progressbars for drawingtime
    private Label labelLeftBar;
    private Label labelRightBar;
    private Label labelBottomBar;
    ...
    public void start(Stage primaryStage) {
        ...
        // ggrid.setGridLinesVisible(true);
        ...
        labelLeftBar = new Label("Left edges: ");
        labelRightBar = new Label("Right edges: ");
        labelBottomBar = new Label("Bottom edges: ");
        grid.add(labelLeftBar, 0, 7);
        grid.add(labelRightBar, 0, 8);
        grid.add(labelBottomBar, 0, 9);

        leftBar = new ProgressBar();
        rightBar = new ProgressBar();
        bottomBar = new ProgressBar();
        leftBar.setPrefWidth(200);
        rightBar.setPrefWidth(200);
        bottomBar.setPrefWidth(200);
        grid.add(leftBar, 3, 7);
        grid.add(rightBar, 3, 8);
        grid.add(bottomBar, 3, 9);
    ...
    public synchronized void drawEdge(Edge e, Color color) {
        // Graphics
        GraphicsContext gc = kochPanel.getGraphicsContext2D();
    ...
    public void requestUpdateLeftProgressBar(Task task) {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                leftBar.progressProperty().bind(task.progressProperty());
            }
        });
    }

    public void requestUpdateRightProgressBar(Task task) {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                rightBar.progressProperty().bind(task.progressProperty());
            }
        });
    }

    public void requestUpdateBottomProgressBar(Task task) {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                bottomBar.progressProperty().bind(task.progressProperty());
            }
        });
    }
